...
float Sign	Exponent	Significand (mantissa)
float 1 [31]	8 [30-23]	23 [22-00]
float SEEEEEEE EMMMMMMM MMMMMMMM MMMMMMMM
https://en.wikipedia.org/wiki/Single-precision_floating-point_format

double Sign	Exponent	Significand (mantissa)
...
class Item_func_int32_bits_to_float : public Item_real_func {
 ...
 public:
  ...
  double val_real();
  const char *func_name() const { return "int32_bits_to_float"; }
  bool fix_length_and_dec() {
    maybe_null= 1; // int may represent an inf or nan
...
class Item_func_int64_bits_to_double : public Item_real_func {
 ...
 public:
  Item_func_int64_bits_to_double(THD *thd, Item *a)
      : Item_real_func(thd, a) {}
  double val_real();
  const char *func_name() const { return "int64_bits_to_double"; }
  bool fix_length_and_dec() {
    maybe_null= 1; // int may represent an inf or nan
    decimals= NOT_FIXED_DEC;
    return FALSE;
